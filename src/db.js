...
e.initdata = d => d ? Promise.resolve(d).then(r => initdocs(r)) :
  httpGet(`${process.env.GITHUB_DB}db.json`).then(r => initdocs(r))

e.backup = () => Promise.all(allDocs.map(get)).then(l => fromPairs(l.map((d, i) => [allDocs[i], d])))
...
e.remove = (doc, obj) => db.collection(doc).remove({ id: obj.id })

e.removeAll = doc => db.collection(doc).deleteMany({})
